...
const RequestorDashboard = ({ user, setUser }) => {
    ...
    const ratedTickets = tickets.filter(t => t.solver_rating_raiser && !isNaN(t.solver_rating_raiser));
    const avgRating = ratedTickets.length > 0
        ? (ratedTickets.reduce((sum, t) => sum + Number(t.solver_rating_raiser), 0) / ratedTickets.length).toFixed(1)
        : 'New';
                                        ...
                                        {filteredTickets.length === 0 ? (
                                            ...
                                            filteredTickets.slice().reverse().map(ticket => (
                                                ...
                                                >
                                                    ...
                                                    <td style={{ padding: '10px', textAlign: 'center', border: '1px solid #27272a' }}>
                                                        <span style={{
                                                            backgroundColor: ticket.status === 'Open' ? 'rgba(245, 158, 11, 0.1)' : ticket.status === 'Resolved' ? 'rgba(16, 185, 129, 0.1)' : ticket.status === 'Closed' ? 'rgba(20, 184, 166, 0.1)' : ticket.status === 'Decline' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(59, 130, 246, 0.1)',
                                                            color: ticket.status === 'Open' ? '#f59e0b' : ticket.status === 'Resolved' ? '#10b981' : ticket.status === 'Closed' ? '#14b8a6' : ticket.status === 'Decline' ? '#ef4444' : '#3b82f6',
                    ...
                    <div style={{
                        ...
                        <div>
                            <h3 style={{ margin: '0 0 4px 0', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <span style={{
                                    backgroundColor: selectedTicket.status === 'Closed' ? '#27272a' : selectedTicket.status === 'Resolved' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(59, 130, 246, 0.1)',
                                    color: selectedTicket.status === 'Closed' ? '#a1a1aa' : selectedTicket.status === 'Resolved' ? '#10b981' : '#60a5fa',
                                    ...
                                    <div style={{ 
                                        ...
                                        {selectedTicket.status === 'Closed' && (
                                            <div>
                                                ...
                                                {selectedTicket.closure_type === 'Declined' ? (
                                                ...
                                                ) : selectedTicket.raiser_rating_solver ? (
                                                    <div style={{ color: '#10b981', fontSize: '11px', backgroundColor: 'rgba(16, 185, 129, 0.1)', padding: '8px', borderRadius: '4px', border: '1px solid rgba(16, 185, 129, 0.2)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                        <Star size={12} fill="#10b981" color="#10b981" /> You rated the solver {selectedTicket.raiser_rating_solver}/5.
                                                    </div>
                                                ) : (
