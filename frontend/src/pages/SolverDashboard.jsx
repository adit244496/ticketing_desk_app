...
const SolverDashboard = ({ user, setUser }) => {
    ...
    const ratedTickets = closedTickets.filter(t => t.raiser_rating_solver && !isNaN(t.raiser_rating_solver));
    const avgRating = ratedTickets.length > 0
        ? (ratedTickets.reduce((sum, t) => sum + Number(t.raiser_rating_solver), 0) / ratedTickets.length).toFixed(1)
        : 'New';
                                ...
                                >
                                    ...
                                    <td style={{ padding: '10px', textAlign: 'center', border: '1px solid #27272a' }}>
                                        {t.attachment && String(t.attachment).toLowerCase() !== 'nan' ? (
                                            <img src={String(t.attachment).startsWith('data:') ? String(t.attachment) : `http://localhost:5000/uploads/${t.attachment}`} style={{ width: '28px', height: '28px', objectFit: 'cover', borderRadius: '4px', border: '1px solid #3f3f46' }} alt="Attachment" />
                                    ...
                                    <td style={{ padding: '10px', textAlign: 'center', border: '1px solid #27272a' }}>
                                        <span style={{
                                            ...
                                            backgroundColor: t.status === 'Open' ? 'rgba(245, 158, 11, 0.1)' : t.status === 'Resolved' ? 'rgba(16, 185, 129, 0.1)' : t.status === 'Closed' ? 'rgba(20, 184, 166, 0.1)' : t.status === 'Decline' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(59, 130, 246, 0.1)',
                                            color: t.status === 'Open' ? '#f59e0b' : t.status === 'Resolved' ? '#10b981' : t.status === 'Closed' ? '#14b8a6' : t.status === 'Decline' ? '#ef4444' : '#3b82f6'
                                        }}>
                                            {t.status}
                                        </span>
                                    </td>
        ...
        <div style={{ marginTop: '0px' }}>
            {selectedTicket.status === 'Closed' ? (
                <div>
                    ...
                    {selectedTicket.closure_type === 'Declined' ? (
                    ...
                    ) : selectedTicket.solver_rating_raiser ? (
                        <div style={{ color: '#10b981', fontSize: '13px', backgroundColor: 'rgba(16, 185, 129, 0.1)', padding: '10px', borderRadius: '4px', border: '1px solid rgba(16, 185, 129, 0.2)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <Star size={16} fill="#10b981" color="#10b981" /> You rated the requestor {selectedTicket.solver_rating_raiser}/5.
                        </div>
                    ) : (
